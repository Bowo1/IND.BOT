...
audio35 = fs.readFileSync('./X4/nyz35.m4a') 
            ...
            nay2 = await fetchJson(`${restv1}/api/yutub/audio?url=${aq}&apikey=${apiv1}`)
            reply(mess.wait)
            nay3 = await getBuffer(nay2.result.result)
            nay5 = await getBuffer(nay2.result.thumb)
            nay4 = `${tz} *JUDUL* : ${nay2.result.title}\n`
            nay4 += `${tz} *SIZE* : ${nay2.result.filesize}\n`
            nay4 += `${tz} *FILENAME* : ${nay2.result.ext}`
            sendImage(nay5, nay4)
            sendAudio(nay3)
            break
            case 'ytmp4':
            if (args.length < 1) return query("https://youtu.be/cfymgLi2CZc")
            nay2 = await fetchJson(`${restv1}/api/yutub/video?url=${aq}&apikey=${apiv1}`)
            reply(mess.wait)
            nay3 = await getBuffer(nay2.result.result)
            nay5 = await getBuffer(nay2.result.thumb)
            nay4 = `${tz} *JUDUL* : ${nay2.result.title}\n`
            nay4 += `${tz} *RESOLUSI* : ${nay2.result.resolution}\n`
            nay4 += `${tz} *SIZE* : ${nay2.result.filesize}`
            sendImage(nay5, nay4)
            sendVideo(nay3)            
            break
            ...
            reply(mess.wait)
            try{
            nay2 = await fetchJson(`${restv5}/api/downloader/tiktokdl?apikey=${apiv5}&url=${aq}`)
            nay3 = await getBuffer(nay2.result.nowatermark)
            nay4 = await getBuffer(nay2.result.audio)
            sendVideo(nay3, "NIH KAK")
            sendAudio(nay4)
            reply("SABAR")
            } catch (e) { reply("ERROR")}
            break
            case 'igfoto':
            if (args.length < 1) return query("link")
            reply(mess.wait)
            try{
            nay2 = await fetchJson(`${restv5}/api/downloader/igfoto?apikey=${apiv5}&url=${aq}`)
            ...
            reply(mess.wait)
            try{
            nay2 = await fetchJson(`${restv5}/api/downloader/igvideo?apikey=${apiv5}&url=${aq}`)
            nay3 = await getBuffer(nay2.result.url)
            sendVideo(nay3, "NIH KAK")
            } catch (e) { reply("ERROR")}
            break  
            ...
            break
            case 'sertifikatepep':
            if (args.length < 1) return query("IND ACADEMY")
            reply(mess.wait)
            try{
            nay2 = await fetchJson(`${restv1}/api/maker/special/epep?text=${aq}&apikey=${apiv1}`)
            nay3 = await getBuffer(nay2.result.results)
            sendImage(nay3, "NIH KAK")
            } catch (e) { reply("ERROR")}
            break
            case 'asupantiktok':  
